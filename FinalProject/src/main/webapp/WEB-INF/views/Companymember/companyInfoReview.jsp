...
$(function (){
	...
	$("#conf").click(function(){
		...
		if($("[id=vWorkarea] > option:selected").val()=="0"){
			alert("근무지역을 선택해주세요");
			return false;
		}
		if($("input:radio[name=vStarscore]").is(":checked")==false){
	...
	$("#conf1").click(function(){
		if($("[id=occup] > option:selected").val()=="0"){
			alert("직종을 선택해주세요");
			return false;
		}
		if($("input:radio[name=vStarscore]").is(":checked")==false){
		...
		if($("[name=vWork] > option:selected").val()=="0"){
			alert("근무지역을 선택해주세요");
			return false;
		}
		if($("input:radio[name=vStarscore]").is(":checked")==false){
...
<form action="insertReview" method="post">
	...
	<input type="hidden" name="memberCode" value="${mbr.memberCode}">
...
<div class="top">
	<p class="back"></p>
	<p class="logo"><img src="resources/images/${companymem.cLogo}"></p>
</div>
	...
	<div class="company-review">
		...
		<c:forEach items="${companyreview}" var="companyreview">
			<div class="company-topreview">
				...
				<c:if test="${mbr.memberCode==companyreview.memberCode}">
				<div class="review-edit">
					<span><a href="#" id="modal_btn1">수정</a></span><span><a href="deleteReview?reviewCode=${companyreview.reviewCode}&companyCode=${companyreview.companyCode}">삭제</a></span>
				</div>
				</c:if>
			</div>
				<form action="updateReview" method="post">
					...
					<table>
						...
						<div>
							<span class="review-class">근무지역</span>
							<span class="review-option">
								<select class="select" name="vWorkarea" id="vWork">
									<option value="0">----근무지역----</option>
